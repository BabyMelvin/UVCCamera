...
public class UVCCamera {
    ...
    protected long mControlSupports;            // 摄像机控件支持的功能标志
...
//    		mNativePtr = 0;	// nativeDestroy 不要在这里清除它，因为调用
        }
        if (mCtrlBlock != null) {
            mCtrlBlock.close();
            mCtrlBlock = null;
    ...
    public static List<Size> getSupportedSize(final int type, final String supportedSize) {
        ...
        if (!TextUtils.isEmpty(supportedSize))
            try {
                ...
                final int formatNums = formats.length();
    ...
    private static final void addSize(final JSONObject format, final int formatType, final int frameType, final List<Size> size_list) throws JSONException {
        ...
        final int sizeNums = size.length();
...
//================================================================================
    ...
    //================================================================================
    public synchronized void setAutoWhiteBalance(final boolean autoWhiteBlance) {
        if (mNativePtr != 0) {
